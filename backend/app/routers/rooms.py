...
async def leave_room(room_id: str, agent_id: str, db: AsyncSession = Depends(get_db)):
...
async def update_room_status(room_id: str, data: RoomStatusUpdate, db: AsyncSession = Depends(get_db)):
    try:
